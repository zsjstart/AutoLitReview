...
literature-search API, you do not search yourself — instead you **emit one
ready-to-run retrieval command** with the concepts already baked in, choosing the
form that fits the user's OS (a curl + jq shell one-shot on macOS/Linux, or a
`fetch_papers.py` invocation on Windows). The user runs it once, gets a
`papers.json` file, and pastes or uploads it back. You perform every other stage.
...
Lock in the final set once they confirm or stay silent. Then emit **one**
copy-paste-and-run retrieval command — but pick the right one for the user's OS.
If you don't already know their OS, ask once (macOS/Linux vs Windows) before
emitting, because the shell command does not run on native Windows.

**Windows users have two equally valid routes** — offer whichever fits what they
already have:
- Install a POSIX shell — **Git Bash, WSL, or MSYS2** — then run **Method A
  exactly as written, unchanged** (same curl + jq, identical to macOS/Linux).
- Or run **Method B** (`fetch_papers.py`), which needs only Python — no shell,
  no jq.
Never hand a bare `cmd` / PowerShell user the Method A command; it won't run there.

**Method A — macOS / Linux (and Windows via Git Bash, WSL, or MSYS2).** The
curl + jq one-shot. Put the confirmed concepts in the `for` list and substitute
parameters:
...
The command works without any API key. If the user has an OpenAlex key (better,
steadier rate limits), add one more line inside the curl call:
`--data-urlencode "api_key=$OPENALEX_API_KEY"`.

**Method B — native Windows, or anyone without bash/jq (cross-platform fallback).**
The `fetch_papers.py` script does the same work in pure Python (no shell quoting,
no jq, writes UTF-8). One line, runs the same on Windows / macOS / Linux:
...
- An OpenAlex API key is optional. Neither command needs one. If the user has a
  key and wants steadier rate limits, they export it once and you add
  `--data-urlencode "api_key=$OPENALEX_API_KEY"` to A (B reads `OPENALEX_API_KEY`
  automatically). Don't make the key a blocker.
...
openalex.org/settings/api, set `OPENALEX_API_KEY` once, and (for Method A) add the
`api_key` line shown above. After emitting the command, proceed to Stage 2 and
wait.
...
One-time prerequisites (mention only the ones for the method you emitted; after
setup the command needs zero edits):
- Method A (curl + jq): `jq` installed — macOS `brew install jq`, Debian/Ubuntu
  `sudo apt install jq`; `curl` (default on macOS/Linux). On Windows this means
  running inside WSL or Git Bash.
- Method B (Python): Python 3 and `pip install requests`. No jq, no shell — runs
  natively on Windows, macOS, and Linux.
- Either way, an OpenAlex API key is optional — both methods run without one. For
  steadier rate limits the user can set it: `export OPENALEX_API_KEY=...` on
  macOS/Linux, or `setx OPENALEX_API_KEY ...` (new terminal) / PowerShell
  `$env:OPENALEX_API_KEY="..."`. Free key at openalex.org/settings/api.
...
- Run the stages in order; show a one-line progress note after each. The natural
  ...
  they run it.
- Emit exactly one command per retrieval, in a single fenced ```bash block, with
  concepts already substituted — nothing for the user to edit. Re-running with
  different parameters means emitting a fresh command, not asking them to tweak.
- Always surface the OpenAlex API key as optional-but-recommended when handing off
  ...
  drop it just because the command runs without a key.
...
## Retrieval methods: curl one-shot vs Python fallback

Two ways to run Stage 2, both writing the same `papers.json` schema:

- **Method A (curl + jq)** — the model bakes concepts + parameters into a single
  shell one-shot. curl fetches OpenAlex; jq reconstructs each abstract from the
  `abstract_inverted_index`, trims to the schema, tags venue quality, and dedups.
  POSIX only — macOS, Linux, or Windows via WSL / Git Bash. Best when you want a
  pure paste-and-run with no saved file.
- **Method B (`fetch_papers.py`)** — same filters, same dedup, in pure Python.
  Runs natively on Windows (no bash, no jq, writes UTF-8), so it's the portable
  default for a mixed audience. Costs a saved file + `pip install requests`.

Pick A for POSIX users who want zero files; pick B for native Windows or anyone
without jq. The jq map and the script both emit the same schema — that schema is
the contract with Stage 2; if you change a field in one, change it everywhere.
